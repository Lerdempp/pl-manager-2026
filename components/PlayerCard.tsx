...
import { getRarityTier, isLegendaryPlayer } from '../services/legendaryPlayers';

interface PlayerCardProps {
  player: Player;
  className?: string;
  previousRating?: number;
  previousAttributes?: PlayerAttributes; // If provided, stats will animate from this to current
  delayAnimation?: boolean;
  cardCount?: number; // Total cards (yellow + red) for this player
  showInjuryStatus?: boolean; // Whether to show injury/illness/suspension status
}
...
export const PlayerCard: React.FC<PlayerCardProps> = ({ 
  ...
  // Helper to render a stat row with potential animation
      ...
      return (
          <div className="flex justify-between items-center relative">
             ...
             <div className="flex items-center gap-1">
                 <span className={`text-lg transition-colors ${valColor}`}>
                    <AnimatedNumber value={currentVal} startValue={prevVal} trigger={startAnim} />
                 </span>
                 {previousAttributes && diff !== 0 && startAnim && (
                     <span className={`text-[10px] font-black absolute -right-4 top-0.5 animate-in fade-in zoom-in duration-500 ${isImproved ? 'text-emerald-400' : 'text-rose-500'}`}>
                         {isImproved ? '+' : ''}{diff}
                     </span>
                 )}
             </div>
  ...
  const rarity = getRarityTier(player.rating);

  // Get beam color based on rarity
  const getBeamColor = () => {
  ...
  // Check if className has custom width/height for pack opening
  const isPackOpening = className.includes('w-[') || className.includes('h-[');
  
  return (
    <div className={`relative w-full h-full ${isPackOpening ? className : 'max-w-[280px]'} aspect-[3/4] preserve-3d`}>
      <div className={`relative w-full h-full aspect-[3/4] rounded-xl border shadow-2xl overflow-visible flex flex-col p-4 select-none transition-transform hover:scale-105 ${cardStyle}`}>
      ...
      <div className="flex justify-between items-center relative z-10 mb-6">
          <div className={`flex items-center gap-2 px-3 py-1 rounded-lg backdrop-blur-sm border border-current/20 ${isGoldOrSilver ? 'bg-black/5' : 'bg-white/10'}`}>
             <div className="flex items-start">
                <span className={`text-3xl font-black leading-none tracking-tighter ${textColorClass}`}>
                    <AnimatedNumber value={player.rating} startValue={previousRating || player.rating} trigger={startAnim} />
                </span>
                {ratingDiff !== 0 && startAnim && (
                    <span className={`text-xs font-bold ml-1 ${ratingDiff > 0 ? 'text-emerald-400' : 'text-rose-500'}`}>
                        {ratingDiff > 0 ? '+' : ''}{ratingDiff}
                    </span>
                )}
             </div>
             <div className="h-6 w-px bg-current opacity-30"></div>
             <span className={`text-sm font-bold uppercase tracking-wider ${textColorClass} opacity-90`}>{translatePosition(player.position)}</span>
          </div>
          {/* Age Pill */}
           <div className={`text-xs font-mono font-bold opacity-60 ${textColorClass}`}>
             {t('age')} {player.age}
           </div>
      </div>
      
      {/* Injury/Illness Status */}
      {showInjuryStatus && (player.injury || player.illness || (player.suspensionGames && player.suspensionGames > 0)) ? (
          <div className="relative z-10 mb-4 flex flex-wrap gap-2 justify-center">
              {player.injury ? (
                  <div className="px-3 py-1.5 bg-red-500/20 border border-red-500/30 rounded-lg">
                      <div className="text-[10px] font-bold text-red-400 uppercase tracking-wider">🏥 {t('injured')}</div>
                      <div className="text-[9px] text-red-300 mt-0.5">{player.injury.type}</div>
                      <div className="text-[9px] text-red-400/80 mt-0.5">{player.injury.weeksOut} {player.injury.weeksOut > 1 ? t('weeksOut') : t('weekOut')}</div>
                  </div>
              ) : null}
              {player.illness ? (
                  <div className="px-3 py-1.5 bg-orange-500/20 border border-orange-500/30 rounded-lg">
                      <div className="text-[10px] font-bold text-orange-400 uppercase tracking-wider">🤒 {t('ill')}</div>
                      <div className="text-[9px] text-orange-300 mt-0.5">{player.illness.type}</div>
                      <div className="text-[9px] text-orange-400/80 mt-0.5">{player.illness.weeksOut} {player.illness.weeksOut > 1 ? t('weeksOut') : t('weekOut')}</div>
                  </div>
              ) : null}
              {player.suspensionGames && player.suspensionGames > 0 ? (
                  <div className="px-3 py-1.5 bg-yellow-500/20 border border-yellow-500/30 rounded-lg">
                      <div className="text-[10px] font-bold text-yellow-400 uppercase tracking-wider">⚠️ {t('suspended')}</div>
                      <div className="text-[9px] text-yellow-400/80 mt-0.5">{player.suspensionGames} {player.suspensionGames > 1 ? t('games') : t('game')}</div>
                  </div>
              ) : null}
          </div>
      ) : null}
      ...
      <div className={`flex-1 flex items-center justify-center relative z-10 mb-6`}>
         <h2 className={`text-3xl font-black uppercase tracking-tight text-center leading-none drop-shadow-md ${textColorClass}`}>
            {player.name || ''}
         </h2>
      </div>

      {/* Stats Grid - Clean & Centered */}
      <div className={`grid grid-cols-2 gap-x-6 gap-y-2 px-2 relative z-10 text-base font-bold ${textColorClass} mt-auto bg-black/5 rounded-lg p-2 backdrop-blur-sm border border-white/5`}>
            ...
            return (
              <div key={traitId} className="relative z-[9999]">
                <button
                  ...
                  className={`px-2 py-1 rounded text-[10px] font-bold border transition-all hover:scale-105 ${rarityColors[rarity || 'common']}`}
                >
                  {getTraitName(traitId)}
                </button>
                {showTooltip && (
                  <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-48 bg-zinc-900 border border-zinc-700 rounded-lg p-3 shadow-xl z-[9999] pointer-events-auto">
                    <div className="text-xs font-bold text-white mb-1">{getTraitName(traitId)}</div>
                    <div className="text-[10px] text-zinc-300">{getTraitDescription(traitId)}</div>
